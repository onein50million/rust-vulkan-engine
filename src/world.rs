use std::{mem::{MaybeUninit}, fmt::Display};

use nalgebra::{Vector3, UnitVector3, Vector4, Vector2};
...
fn production_curve(population:f64, curviness: f64,base_slope: f64, slope_falloff: f64, current_supply: f64) -> f64{
    let b = 1.0 / slope_falloff;
    let output = base_slope * (population-(curviness*(b*b*population*population+1.0).sqrt())/b) + curviness*slope_falloff*base_slope;
...
fn get_outputs(industry: Industry, population: f64, delta_year: f64, current_supply: f64) -> [f64; Good::VARIANT_COUNT]{

    let mut output = [0.0; Good::VARIANT_COUNT];

    match industry{
        Industry::Farm => {
            output[Good::Grain as usize] = production_curve(population, 0.5, 1.0, 5000.0,current_supply) * delta_year;
        },
        Industry::Mill => {
            output[Good::Food as usize] = production_curve(population, 1.0, 60.0, 100.0,current_supply) * delta_year;
        },
        Industry::Mine => {
            output[Good::Ore as usize] = production_curve(population, 1.0, 6.5, 1000.0,current_supply) * delta_year;
        },
        Industry::Smelter => {
            output[Good::Metal as usize] = production_curve(population, 1.0, 1000.0, 100.0,current_supply) * delta_year;
        },
    }
    ...
    pub fn new(total_population:f64) -> Self{
        ...
        unsafe{
            ...
            for i in 0..SLICE_COUNT{
                (pop_slices.as_mut_ptr() as *mut PopSlice).offset(i as isize).write(PopSlice{
...
impl Display for Province{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for i in 0..Good::VARIANT_COUNT{
            out_string.push_str(format!("Good: {:?}, Supply: {:.2}, Demand: {:.2}, Price: {:.2}\n", Good::try_from(i).unwrap(),
            self.market.supply[i],
            self.market.demand[i],
            self.market.price[i]).as_str());
        }
        ...
        for (i, slice) in self.pops.pop_slices.iter().enumerate(){
            ...
            out_string.push_str(format!("Culture: {:?}, Building: {:?}, Population: {:.2}, Money: {:.2}\n",
            culture,
            industry,
            slice.population,
            slice.money).as_str());
            ...
            for i in 0..Good::VARIANT_COUNT{
                out_string.push_str(format!("   Good: {:?}, Amount: {:.2}\n",Good::try_from(i).unwrap(), slice.owned_goods[i]).as_str());
            }
        }
...
impl World{
    ...
    pub fn new() -> Self{
        Self{
            provinces: [
                Province{ point_indices: vec![0,1,2], pops: Pops::new(10_000.0),
                    market: Market {
                         price: [1.0; Good::VARIANT_COUNT], supply: [0.0; Good::VARIANT_COUNT], demand: [0.0; Good::VARIANT_COUNT],
                         previous_supply_demand_error: [0.0; Good::VARIANT_COUNT],
                         supply_demand_error_integral: [0.0; Good::VARIANT_COUNT] },
                },
                Province{ point_indices: vec![0,3,4], pops: Pops::new(1_000.0),
                    market: Market {
    ...
    pub fn get_indices(&self) -> Vec<usize>{
        self.provinces.iter().flat_map(|province|{
            province.point_indices.iter().map(|index| *index)
        }).collect()
    }
    ...
    pub fn process(&mut self, delta_year: f64){
        for (province_index, province) in self.provinces.iter_mut().enumerate(){
            ...
            for (index, slice) in province.pops.pop_slices.iter_mut().enumerate(){
                ...
                let goods_needed: Vec<_> = get_inputs(industry, slice.population, delta_year).iter().zip(get_needs(slice.population, delta_year).iter()).map(|(a,b)| a+b).collect();
                ...
                for i in 0..Good::VARIANT_COUNT{
                    ...
                    let good_needed_to_be_bought = (good_needed - slice.owned_goods[good as usize]).max(0.0);
                    

                    let affordable_good = (slice.money / num_distinct_goods_wanted as f64) / province.market.price[good as usize];
            ...
            for good_index in 0..Good::VARIANT_COUNT{
                ...
                for (index, slice) in province.pops.pop_slices.iter_mut().enumerate(){
                    ...
                    let sell_ratio = if province.market.supply[good as usize] > 0.0 {
                        (province.market.demand[good as usize] / province.market.supply[good as usize]).min(1.0)
                    }else{
                        0.0
                    };
                    ...
                    let buy_ratio = if province.market.demand[good as usize] > 0.0 {
                        (province.market.supply[good as usize] / province.market.demand[good as usize]).min(1.0)
                    }else{
                        0.0
                    };
                ...
                let supply_demand_delta = province.market.supply[good as usize] - province.market.demand[good as usize];
                ...
                let derivative = (error - province.market.previous_supply_demand_error[good as usize]) / delta_year;
                let output = PRICE_PID_P * proportional + PRICE_PID_I * province.market.supply_demand_error_integral[good as usize] + PRICE_PID_D * derivative;
                province.market.previous_supply_demand_error[good as usize] = error;

                province.market.price[good as usize] += output.clamp(-0.1*province.market.price[good as usize],0.1*province.market.price[good as usize]);
                province.market.price[good as usize] = province.market.price[good as usize].max(0.01);
            ...
            for slice in province.pops.pop_slices.iter_mut(){
                ...
                let minimum_met_need = needs.iter().enumerate().map(|(good_index,need_amount)|{
                ...
                if minimum_met_need > 0.5{
                    ...
                    slice.population -= (slice.population * 0.01 * delta_year).min(slice.population);
                }
                if minimum_met_need < 0.9{
                    let migration_amount = (slice.population * 0.1 * delta_year).min(slice.population);
            ...
            for (index, slice) in province.pops.pop_slices.iter_mut().enumerate(){
                ...
                let minimum_met_input = inputs.iter().enumerate().map(|(good_index,need_amount)|{
                    // println!("good: {:?}, amount: {:}",Good::try_from(good_index).unwrap() , need_amount);
                    if need_amount <= &0.0{
                        1.0
                    }else{
                ...
                //     }    
                // }
                let outputs = get_outputs(industry, slice.population,delta_year, slice.owned_goods.into_iter().reduce(f64::max).unwrap());
                for (good_index,output) in outputs.iter().enumerate(){
                    let good = Good::try_from(good_index).unwrap();
                    slice.owned_goods[good as usize] -= inputs[good as usize] * minimum_met_input;
                    slice.owned_goods[good as usize] += output * minimum_met_input;
