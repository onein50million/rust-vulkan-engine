...
impl Octree{
    ...
    fn add_node(&mut self,parent_index: u32, branch_index: u32, node_type: u32) -> u32 {
        ...
        assert_ne!(self.nodes[parent_index as usize].node_type, node_types::REFLECTIVE);
        self.nodes[parent_index as usize].node_type = node_types::CHILDREN;
        assert_eq!(self.nodes[parent_index as usize].leaf_indices[branch_index as usize], 0); //make sure branch isn't already assigned
    ...
    fn get_next_deeper_node(&self, node_index: u32, mut visited: Vec<u32>) -> (u32, Vec<u32>){
        ...
        loop{
            for current_branch in 0..8 {
                let next_node_index = self.nodes[current_index as usize].leaf_indices[current_branch as usize];
                if next_node_index != 0 && !visited.contains(&next_node_index) && self.nodes[next_node_index as usize].node_type == node_types::CHILDREN {
    ...
    pub(crate) fn calculate_nexts(&mut self){
        ...
        loop {
            let (next_node, updated_next_visited) = self.get_next_deeper_node(current_node_index, next_visited);
            next_visited = updated_next_visited;
            assert_eq!(self.nodes[next_node as usize].node_type, node_types::CHILDREN);
            let (next_same_level_node, updated_same_visited) = self.get_next_same_node(current_node_index, same_visited);
            assert_eq!(self.nodes[next_same_level_node as usize].node_type, node_types::CHILDREN);
