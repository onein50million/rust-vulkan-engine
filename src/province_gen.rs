...
impl <'a> Islands<'a>{
    pub fn new(province_points: &'a [(Vector3<f32>, f32)], planet_mesh: &MeshOutput) -> Self{
        let islands: Vec<_> = province_points.iter().enumerate().map(|(index, _value)| {
        ...
        Self{
            ...
            planet_mesh_vertices: planet_mesh.vertices.iter().map(|&vertex|(vertex.position, vertex.elevation)).collect(),
            ...
            for other_island_index in (island_index+1)..self.islands.len(){
                ...
                // for &point_index in &self.islands[island_index]{
                    for &other_point_index in &self.islands[other_island_index]{
                        let contiguous = are_points_contiguous(self.points[point_index].0, self.points[other_point_index].0, &self.planet_mesh_vertices, &self.planet_mesh_indices);
    ...
    //     if first_island.len() > second_island.len(){
    //         first_island.extend(second_island.iter());
    //         self.islands.remove(second_index);
    //     }else{
    //         second_island.extend(first_island.iter());
    //         self.islands.remove(first_index);
    //     }
    // }

    fn get_two_islands(&mut self, first_index: usize, second_index: usize) -> (&mut HashSet<usize>,&mut HashSet<usize>){
        unsafe{
            return (&mut *(&mut self.islands[first_index] as *mut _), &mut *(&mut self.islands[second_index] as *mut _))
        }
    }
    ...
    //         let point_indices: Vec<_> = island.iter().collect();
    ...
    // fn split(&mut self, index:usize) -> (&mut HashSet<usize>, &[HashSet<usize>]){
    //     let first_island = &{
    //         for island in self.islands
    //     }
    // }
    // fn remove_point(&mut self, index: usize) -> usize{
    //     for (island_index, island) in self.islands.iter_mut().enumerate(){
    //         if island.remove(&index){
    //             if island.is_empty(){
