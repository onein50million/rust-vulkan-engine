use std::{fs::{read_dir}, collections::{HashMap, hash_map::Entry, HashSet}};
use float_ord::FloatOrd;
use gdal::errors::GdalError;
use genmesh::generators::{IcoSphere, SharedVertex, IndexedPolygon};
...
pub fn get_planet(radius: f32) -> MeshOutput{
    ...
    let mut vertices: Vec<_> = sphere.shared_vertex_iter().map(|vertex|{
        Vertex {
    ...
    let mut raw_vertex_data = vec![RawVertexData{ sum: 0.0, sample_count: 0, data_priority: 0 }; vertices.len()];
    for folder in paths.filter_map(|s| s.ok()){
        let data_priority =
        if folder.file_name().to_str().unwrap() == "arctic" {
            1
        } else {
            0
        };
        ...
        let path = image_files.filter_map(|s|s.ok()).find(|image_file|{
        ...
        for (i, (vertex, raw_vertex)) in vertices.iter().zip(raw_vertex_data.iter_mut()).enumerate(){
            ...
            let latitude = (vertex.position.y
                .asin() as f64)
                .to_degrees();
                
            let longitude = (vertex.position.z
                .atan2(vertex.position.x)
                as f64)
                .to_degrees();
    ...
    let indices: Vec<_> = sphere.indexed_polygon_iter().map(|triangle|{
        [triangle.x, triangle.y, triangle.z].into_iter()
    }).flatten().collect();

    let mut vertex_data: Vec<_> = vertices.iter().enumerate().map(|(index, vertex)| {
        ...
        VertexData{
            ...
            neighbours: closest_vertices.iter().map(|(index, _)| *index).take(NUM_NEIGHBOURS).collect(),
        }
    }
    ).collect();
    ...
    let min_elevation = vertices.iter().fold(f32::INFINITY, |a, &b| {
        a.min(b.elevation)
    });
    let max_elevation = vertices.iter().fold(f32::NEG_INFINITY, |a, &b| {
        a.max(b.elevation)
    });

    for vertex in &mut vertex_data{
        vertex.quantized_elevation = ((vertex.elevation/max_elevation.max(min_elevation.abs())) * 128.0) as i8
    }
    ...
    loop {
        ...
        for(index, (vertex, cell)) in vertices.iter_mut().zip(wave_function_collapse.cells.iter()).enumerate(){
            match cell.cell_state{
                CellState::Collapsed(quantized_elevation) => {
                    vertex.elevation = (quantized_elevation as f32 / 128.0) * max_elevation.max(min_elevation.abs());
        ...
        if unfinished_cell_indices.len() > 0{
            ...
            wave_function_collapse.collapse_cell(*unfinished_cell_indices.iter().nth(rand_index).unwrap());
        }else{
            break;
        }
    }
    ...
    for triangle_index in (0..indices.len()).step_by(3) {
        ...
        for i in 0..3 {
            vertices[indices[triangle_index + i] as usize]
...
impl ElevationWaveFunctionCollapse{
    fn new(vertex_data: &[VertexData]) -> Self{
        ...
        for i in 0..vertex_data.len(){
            let inner_map = elevation_mapping.entry(vertex_data[i].quantized_elevation).or_insert(HashSet::new());
        ...
        let cells: Vec<_> = vertex_data.iter().map(|vertex|{
            ...
            Cell { cell_state: CellState::Superposition(superpositions), neighbours: vertex.neighbours.clone() }
        }).collect();
    ...
    fn get_possible_states(&self, index: usize) -> HashSet<i8>{
        match &self.cells[index].cell_state{
            CellState::Collapsed(state) => {
                self.elevation_mapping[&state].clone()
            },
            ...
            for i in 0..self.cells.len(){
                let new_states = match &self.cells[i].cell_state{
                    ...
                    CellState::Superposition(states) => {
                        ...
                        let out: HashSet<_> = new_states.intersection(states).map(|&state| state).collect();
                        if &out != states{
                            done = false;
                        }
                        out
                ...
                if new_states.len() == 0{
                    ...
                    self.cells[i].cell_state = CellState::Collapsed(*new_states.iter().next().unwrap())
                }else{
    ...
    fn collapse_cell(&mut self, index: usize){
        match &self.cells[index].cell_state{
            ...
            CellState::Superposition(superpositions) => {
                ...
                self.cells[index].cell_state = CellState::Collapsed(*superpositions.iter().nth(random).unwrap());
